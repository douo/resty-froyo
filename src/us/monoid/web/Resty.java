...
import info.dourok.net.CookieHandler;
import info.dourok.net.CookieKit;
import info.dourok.net.CookieManager;
import info.dourok.net.HttpCookie;
import info.dourok.net.InMemoryCookieManager;
 ...
 * methods, like json(...), xml(...), text(...) or bytes(...).
 * 
 * The idea is that the method name will convey the expected content type you
 * can then operate on. Most static methods help you build content objects or
 * queries with a compact syntax. Static methods like put(...) and delete() are
 * used to implement the respective HTTP methods.
 * 
 * A neat trick to save you typing is to use
 * 
 * <pre>
 * <code>import static us.monoid.web.Resty.*;</code>
 * </pre>
 * <p>
 * 
 ...
 * 
 * <pre>
 * <code>new Resty().json(url);</code>
 * </pre>
 * 
 * POSTing to an URL (using multipart/form-data) and expecting JSON back:
 * 
 * <pre>
 * <code>new Resty().json(url, form(data("name", "Don Draper"), data("occupation", "Ad Man")));</code>
 * </pre>
 * 
 ...
 * 
 * <pre>
 * <code>new Resty().json(url, delete());</code>
 * </pre>
 * 
 * 
 ...
 * create your own options! See Resty.Option for more info.
 * 
 * @author beders
 * 
 */
...
public class Resty {
	...
	static {
		...
		// 将使用独立的应用程序级别的CookieManager
//		try {
//			CookieHandler.setDefault(new CookieManager());
//		} catch (NoClassDefFoundError oops) {
//			System.err
//					.println("No CookieHandler. Running on GAE? Fine. No cookie support for you!");
//		}
	...
	//private static InMemoryCookieManager cookieManager = new InMemoryCookieManager();;

	/**
	 * Create an instance of Resty with the following list of options. Also,
	 * options are carried over to resources created by calls to
	 * json/text/binary etc. Use {@link #setOptions(Option...)} to change
	 * options afterwards.
	 * 
	 */
	...
	/**
	 * Register this root URI for authentication. Whenever a URL is requested
	 * that starts with this root, the credentials given are used for HTTP AUTH.
	 * Note that currently authentication information is shared across all Resty
	 ...
	 *            the login name
	 * @param aPwd
	 *            the password. The array will not be internally copied.
	 *            Whenever you null it, the password is gone within Resty
	 */
	...
	/**
	 * Register a login password for the realm returned by the authorization
	 * challenge. Use this method instead of authenticate in case the URL is not
	 * made available to the java.net.Authenticator class
	 * 
	 * @param realm
	 ...
	 * @param charArray
	 */
	public void authenticateForRealm(String realm, String aLogin,
			char[] charArray) {
	 ...
	 */
	public JSONResource json(URI anUri, AbstractContent requestContent)
			throws IOException {
		return doPOSTOrPUT(anUri, requestContent, createJSONResource());
	}

	/** @see Resty#json(URI, Content) */
	public JSONResource json(String anUri, AbstractContent content)
			throws IOException {
	 ...
	 *             could not be made and gazillion other reasons
	 */
	 ...
	 *             could not be made and gazillion other reasons
	 */
	public TextResource text(URI anUri, AbstractContent content)
			throws IOException {
	 ...
	 *             could not be made and gazillion other reasons
	 */
	public TextResource text(String anUri, AbstractContent content)
			throws IOException {
	 ...
	 *             zillion other reasons
	 */
	public XMLResource xml(URI anUri, AbstractContent requestContent)
			throws IOException {
		return doPOSTOrPUT(anUri, requestContent, createXMLResource());
	}

	/** @see Resty#xml(URI, Content) */
	public XMLResource xml(String anUri, AbstractContent content)
			throws IOException {
	 ...
	 * @throws IOException
	 */
	public BinaryResource bytes(String anUri, AbstractContent someContent)
			throws IOException {
	 ...
	 * @throws IOException
	 */
	public BinaryResource bytes(URI anUri, AbstractContent someContent)
			throws IOException {
		return doPOSTOrPUT(anUri, someContent, createBinaryResource());
	}

	protected <T extends AbstractResource> T doGET(URI anUri, T resource)
			throws IOException {
		URLConnection con = openConnection(anUri, resource);
		return fillResourceFromURL(con, resource);
	}

	protected <T extends AbstractResource> T doPOSTOrPUT(URI anUri,
			AbstractContent requestContent, T resource) throws IOException {
		URLConnection con = openConnection(anUri, resource);
		requestContent.addContent(con);
		return fillResourceFromURL(con, resource);
	}

	protected <T extends AbstractResource> URLConnection openConnection(
			URI anUri, T resource) throws IOException, MalformedURLException {
		URLConnection con = anUri.toURL().openConnection(proxy);
		addStandardHeaders(con, resource);
		addAdditionalHeaders(con);
		// FIXME 提取出cookieManager的接口
//		cookieManager.setCookies(con);
		try {
			CookieKit.addCookies(con);
	...
	protected void addAdditionalHeaders(URLConnection con) {
		for (Map.Entry<String, String> header : getAdditionalHeaders()
				.entrySet()) {
	...
	protected <T extends AbstractResource> void addStandardHeaders(
			URLConnection con, T resource) {
	 ...
	 * @throws IOException
	 */
	protected <T extends AbstractResource> T fillResourceFromURL(
			URLConnection con, T resource) throws IOException {
		// FIXME 获取Cookiemanager
		// con.connect();
		resource.fill(con);
		resource.getAdditionalHeaders().putAll(getAdditionalHeaders()); // carry
																		// over
																		// additional
																		// headers
																		// TODO
...
//		cookieManager.storeCookies(con);
	...
	/**
	 * Create a JSONPathQuery to extract data from a JSON object. This is
	 * usually used to extract a URI and use it in
	 * json|text|xml(JSONPathQuery...) methods of JSONResource. <code>
	 * Resty r = new Resty();
	...
	/**
	 * Create an XPathQuery to extract data from an XML document. This is
	 * usually used to extract a URI and use it in json|text|xml(XPathQuery...)
	 * methods. In this case, your XPath must result in a String value, i.e. it
	 * can't just extract an Element.
	 * 
	...
	public static Content content(JSONObject someJson) {
		Content c = null;
		try {
			c = new Content("application/json; charset=UTF-8", someJson
	...
	/**
	 * Create a content object from plain text. Use this to POST the content to
	 * a URL.
	 * 
	 * @param somePlainText
	...
	public static Content content(String somePlainText) {
		Content c = null;
		try {
			c = new Content("text/plain; charset=UTF-8",
					somePlainText.getBytes("UTF-8"));
		} catch (UnsupportedEncodingException e) { /*
													 * UTF-8 is never
													 * unsupported
													 */
	...
	/**
	 * Create a content object from a byte array. Use this to POST the content
	 * to a URL with mime type application/octet-stream.
	 * 
	 * @param bytes
	 *            the bytes to send
	 * @return the content to send
	...
	/**
	 * Create form content to be sent as multipart/form-data. Useful if you want
	 * to upload files or have tons of form data that looks really ugly in a
	 * URL.
	 * 
	 * 
	 */
	 ...
	 * PUT operation instead of a POST operation. Example use: r.json(uri,
	 * put(content("bubu")));
	 */
	 ...
	 * DELETE. Example use: r.json(uri,delete());
	 */
	 ...
	 * instance. These headers will also be sent from any resource object
	 * returned by this instance. I.e. chained calls will carry over the headers
	 * r.json(url).json(get("some.path.to.a.url")); Multiple headers of the same
	...
	/**
	 * Install a SSL socket factory which will trust all certificates. This is
	 * generally considered dangerous as you won't know if you are really
	 * talking to the server you think you are talking to. This is like your
	 * best friend using his Facebook account in an Apple store and forgetting
	 * to log out.
	 * 
	 * This method should be called once and replaces the SSL factory for ALL
	 * HttpsURLConnections.
	 */
	...
	/**
	 * Set the proxy for this instance of Resty. Note, that the proxy settings
	 * will be carried over to Resty instances created by this instance only if
	 * you used new Resty(Option.proxy(...))!
	 * 
	 * @param proxyhost
	 *            name of the host
	 * @param proxyport
	 ...
	 * Override one of the apply methods to change an object like URLConnection
	...
	abstract public static class Option {
		 ...
		 * Called before any connection is made. Override to set initial
		 * properties
		 */
		 ...
		 * <code><pre> new Resty(Option.timeout(3000)); </pre></code>
		 * 
	...
	/**
	 * Option to set a timeout. Use the static timeout method for added
	 * convenience.
	 */
